...
const testUseManyAndAppendOne = async (indices, seed, contract) => {
  const gasFixtureString = `${contract.type}_${contract.elementCount}_testUseManyAndAppendOne_${indices.join('-')}_${seed}`;
  const expectedGas = gasCosts[gasFixtureString];
...
const testUseManyAndAppendMany = async (indices, appendSize, seed, contract) => {
  const gasFixtureString = `${contract.type}_${contract.elementCount}_testUseManyAndAppendMany_${indices.join('-')}_${appendSize}_${seed}`;
  const expectedGas = gasCosts[gasFixtureString];
...
const testUpdateOneAndAppendOne = async (index, updateSeed, appendSeed, contract) => {
  ...
  const hexAppendElement = '0x' + appendElement.toString('hex');
  const { newMerkleTree, proof } = contract.merkleTree.updateAndAppend(index, updateElement, appendElement, contract.proofOptions);
  const { element, compactProof } = proof;
  const hexElement = '0x' + element.toString('hex');
  const hexProof = compactProof.map(p => '0x' + p.toString('hex'));
  const { receipt } = await contract.instance.update_one_and_append_one(index, hexElement, hexUpdateElement, hexAppendElement, hexProof);
  contract.merkleTree = newMerkleTree;
...
const testUpdateOneAndAppendMany = async (index, updateSeed, appendSize, appendSeed, contract) => {
  ...
  const hexAppendElements = appendElements.map(e => '0x' + e.toString('hex'));
  const { newMerkleTree, proof } = contract.merkleTree.updateAndAppend(index, updateElement, appendElements, contract.proofOptions);
  const { element, compactProof } = proof;
  const hexElement = '0x' + element.toString('hex');
  const hexProof = compactProof.map(p => '0x' + p.toString('hex'));
  const { receipt } = await contract.instance.update_one_and_append_many(index, hexElement, hexUpdateElement, hexAppendElements, hexProof);
  contract.merkleTree = newMerkleTree;
...
const testUpdateManyAndAppendOne = async (indices, updateSeed, appendSeed, contract) => {
  const gasFixtureString = `${contract.type}_${contract.elementCount}_testUpdateOneAndAppendMany_${indices.join('-')}_${updateSeed}_${appendSeed}`;
  const expectedGas = gasCosts[gasFixtureString];
  ...
  const appendElement = generateElements(1, { seed: appendSeed })[0];
  const hexAppendElement = '0x' + appendElement.toString('hex');
  const { newMerkleTree, proof } = contract.merkleTree.updateAndAppend(indices, updateElements, appendElement, contract.proofOptions);
  const { elements, compactProof } = proof;
  const hexElements = elements.map(e => '0x' + e.toString('hex'));
  const hexProof = compactProof.map(p => '0x' + p.toString('hex'));
  const { receipt } = await contract.instance.update_many_and_append_one(hexElements, hexUpdateElements, hexAppendElement, hexProof);
  contract.merkleTree = newMerkleTree;
...
const testUpdateManyAndAppendMany = async (indices, updateSeed, appendSize, appendSeed, contract) => {
  const gasFixtureString = `${contract.type}_${contract.elementCount}_testUpdateManyAndAppendMany_${indices.join('-')}_${updateSeed}_${appendSize}_${appendSeed}`;
  const expectedGas = gasCosts[gasFixtureString];
  ...
  const appendElements = generateElements(appendSize, { seed: appendSeed });
  const hexAppendElements = appendElements.map(e => '0x' + e.toString('hex'));
  const { newMerkleTree, proof } = contract.merkleTree.updateAndAppend(indices, updateElements, appendElements, contract.proofOptions);
  const { elements, compactProof } = proof;
  const hexElements = elements.map(e => '0x' + e.toString('hex'));
  const hexProof = compactProof.map(p => '0x' + p.toString('hex'));
  const { receipt } = await contract.instance.update_many_and_append_many(hexElements, hexUpdateElements, hexAppendElements, hexProof);
  contract.merkleTree = newMerkleTree;
