...
const gasCosts = require('./fixtures/merkle-gas-costs-calldata.json');
const { generateElements } = require('./helpers');
const { MerkleTree } = require('../../js');

const MerkleStorageUsingLibCalldata = artifacts.require("Merkle_Storage_Using_Lib_Calldata");
...
describe.only("Merkle Storage Using Merkle Library", async () => {
  after(() => {
    fs.writeFileSync('./test/fixtures/merkle-gas-costs-calldata.json', JSON.stringify(gasCosts, null, ' ').concat('\n'));
  });

  describe("Starting with 0 elements (Unsorted Hash)", async () => {
    beforeEach(async () => {
      contractInstance = await MerkleStorageUsingLibCalldata.new();
  ...
  describe.only("Starting with 200 elements (Unsorted Hash)", async () => {
    beforeEach(async () => {
      contractInstance = await MerkleStorageUsingLibCalldata.new();
      const seed = 'ff';
      const elements = [];
