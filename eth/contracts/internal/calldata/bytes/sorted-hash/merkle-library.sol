...
library Merkle_Library_ICBSH {
  ...
  function get_nodes_from_elements(bytes[] calldata elements)
    internal
    pure
  ...
  ) internal pure returns (uint256 leaf_count, bytes32[] memory leafs) {
    leaf_count = elements.length;
  ...
  function get_leafs_from_elements(bytes[] calldata elements)
    internal
    pure
    returns (uint256 leaf_count, bytes32[] memory leafs)
  {
  ...
  function create_from_one(bytes calldata element) internal pure returns (bytes32 new_element_root) {
    return hash_node(bytes32(uint256(1)), get_root_from_one(element));
  }

  // Create a tree and return the root, given many element
  function create_from_many(bytes[] calldata elements) internal pure returns (bytes32 new_element_root) {
