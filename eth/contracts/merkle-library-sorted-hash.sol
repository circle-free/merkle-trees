...
library Merkle_Library_Sorted_Hash {
  ...
  function get_roots_from_multi_proof_update(bytes32[] memory elements, bytes32[] memory update_elements, bytes32[] memory proof) internal pure returns (bytes32 flags, bytes32 skips) {
    ...
    bytes32[] memory hashes = new bytes32[](update_element_count);
    bytes32[] memory update_hashes = new bytes32[](update_element_count);
  ...
  function get_append_proof_from_multi_proof_update(bytes32[] memory elements, bytes32[] memory update_elements, bytes32[] memory proof) internal pure returns (bytes32 append_hash, bytes32[] memory append_proof) {
    uint256 update_element_count = update_elements.length;
    require(elements.length == update_element_count, "LENGTH_MISMATCH");
    ...
    while (true) {
      ...
      if (flags & bit_check == bit_check) {
        ...
        hashes[write_index] = hash_pair(proof[proof_index], hashes[read_index]);
        hashes[update_element_count + write_index] = hash_pair(proof[proof_index], hashes[update_element_count + read_index]);
        proof_index += 1;
        read_index += 1;
      }
