...
library Merkle_Library_DMB32S {
  ...
  function get_indices(bytes32[] memory elements, bytes32[] memory proof) external pure returns (uint256[] memory) {
  ...
  function create_from_many(bytes32[] memory elements) external pure returns (bytes32 new_element_root) {
