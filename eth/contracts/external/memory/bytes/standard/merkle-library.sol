...
library Merkle_Library_DMBS {
  ...
  function get_indices(bytes[] memory elements, bytes32[] memory proof) external pure returns (uint256[] memory) {
  ...
  function create_from_one(bytes memory element) external pure returns (bytes32 new_element_root) {
    return hash_node(bytes32(uint256(1)), get_root_from_one(element));
  }

  // Create a tree and return the root, given many element
  function create_from_many(bytes[] memory elements) external pure returns (bytes32 new_element_root) {
