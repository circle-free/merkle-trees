...
export const generate = (tree: Array<Buffer>, elementCount: number, indices: Array<number>, options: proofOptions = defaultProofOptions): proof => {
  const known = Array(tree.length).fill(false)
  const decommitments = []
  const leafCount = tree.length >>> 1

  for (let i = 0; i < indices.length; i++) {
    if (i !== 0 && indices[i - 1] > indices[i]) throw new Error('Indices must be in ascending order.')
...
export const getNewRoot = (params: getNewRootParams, options: treeOptions = defaultTreeOptions): { root: Buffer, newRoot: Buffer, elementCount: number } => {
  ...
  while (true) {
    ...
    if (nodeIndex === upperBound && !indexIsOdd) {
      ...
      treeIndices[writeIndex] = nodeIndex >>> 1
      hashes[writeIndex] = options.hashFunction(left, right)
      updateHashes[writeIndex++] = options.hashFunction(newLeft, newRight)
    }
...
export const getPartialTree = (indices: Array<number>, leafs: Array<Buffer>, compactProof: Array<Buffer>, elementCount: number, decommitments: Array<Buffer>, options: treeOptions = defaultTreeOptions): { tree: Array<Buffer>, elementCount: number } => {
